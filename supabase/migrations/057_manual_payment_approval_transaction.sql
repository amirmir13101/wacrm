-- ============================================================
-- 057_manual_payment_approval_transaction.sql
-- Approves a hosted Pro manual payment atomically. PostgreSQL rolls
-- every update back if any statement in this function fails.
-- ============================================================
...
BEGIN
  ...
  IF target_workspace_id IS NULL THEN
    ...
    RETURNING workspaces.id INTO target_workspace_id;
  END IF;
  ...
  RETURN QUERY
  SELECT request.id, request.status, request.workspace_id
  FROM public.manual_payment_requests request
  WHERE request.id = p_request_id;
END;
$$;
