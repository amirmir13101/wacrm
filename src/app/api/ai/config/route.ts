...
export async function POST(request: Request) {
  try {
    ...
    const shared: Record<string, unknown> = {
      provider,
      model,
      // TalkWagon migration 065 keeps this column NOT NULL. The
      // upstream AI Agent treats the business-specific prompt as
      // optional, so persist an empty string when the form is blank;
      // buildSystemPrompt treats it exactly like null while the write
      // remains compatible with the workspace schema.
    ...
    if (existing) {
      ...
      const { error: insErr } = await supabase.from('ai_agent_configs').insert({
