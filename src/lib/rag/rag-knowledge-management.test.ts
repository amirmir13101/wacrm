...
describe('RAG manual knowledge management', () => {
  ...
  it('adds the customer-facing Knowledge Base UI without technical internals', () => {
    ...
    expect(page).toContain('Embeddings are pending. Click Prepare for Chatbot when ready.')
  ...
  it('adds workspace-scoped CRUD routes with the right permissions', () => {
    ...
    expect(listRoute).not.toContain('embedRagManualKnowledgeSource')
    ...
    expect(detailRoute).not.toContain('embedRagManualKnowledgeSource')
