...
async function loadActiveRunForContact(
  ...
  // rebuilt in migration 017 over `(workspace_id, contact_id)` — so
...
async function isDuplicateInbound(
  ...
  // Fetch ALL run ids for this contact in this workspace (active +
...
async function findEntryFlow(
  ...
  // Pull all active flows for this workspace. Active set is bounded
...
async function startNewRun(
  ...
  const { data: inserted, error: insErr } = await db
    ...
    .insert({
      ...
      // `idx_one_active_run_per_contact` is over (workspace_id,
